...
export default function Reports() {
  ...
  async function loadReport() {
    if (!startDate || !endDate) return setPageError("Tanggal mulai dan tanggal akhir wajib diisi.");
    if (startDate > endDate) return setPageError("Tanggal mulai tidak boleh melewati tanggal akhir.");
    try {
  ...
  function handleExport(kind) {
    if (!report) return setPageError("Tampilkan laporan terlebih dahulu sebelum export.");
    try {
      setPageError("");
  ...
  const selectedLabel = REPORT_TYPES.find((item) => item.value === reportType)?.label || "Laporan";
  const incomeColumns = [
  ...
  const productionColumns = [
    { key: "tanggal", title: "Tanggal", render: (item) => formatDate(item.tanggal) },
    { key: "kode", title: "Batch" }, { key: "recipe", title: "Recipe" },
    ...
    { key: "reject", title: "Reject", render: (item) => `${formatNumber(item.reject)} pcs` }, { key: "status", title: "Status" },
  ];

  if (loading && !report) return <LoadingState message="Memuat laporan..." />;
  let table = null; let totalLabel = "Total"; let totalValue = 0;
  if (report) {
    if (reportType === "income") { table = <DataTable columns={incomeColumns} data={report.incomes} emptyMessage="Tidak ada pemasukan pada periode ini" />; totalLabel = "Total Pemasukan"; totalValue = report.summary.totalIncome; }
    else if (reportType === "expense") { table = <DataTable columns={expenseColumns} data={report.expenses} emptyMessage="Tidak ada pengeluaran pada periode ini" />; totalLabel = "Total Pengeluaran"; totalValue = report.summary.totalExpense; }
    else if (reportType === "purchase") { table = <DataTable columns={purchaseColumns} data={report.purchases} emptyMessage="Tidak ada purchasing pada periode ini" />; totalLabel = "Total Purchasing"; totalValue = report.summary.totalPurchase; }
    else if (reportType === "sales") { table = <DataTable columns={salesColumns} data={report.sales} emptyMessage="Tidak ada penjualan pada periode ini" />; totalLabel = "Total Penjualan"; totalValue = report.summary.totalSales; }
    else if (reportType === "stock") table = <DataTable columns={stockColumns} data={report.stock} emptyMessage="Tidak ada pergerakan stok pada periode ini" />;
    else if (reportType === "product_out") { table = <DataTable columns={productOutColumns} data={report.productOut} emptyMessage="Tidak ada produk keluar pada periode ini" />; totalLabel = "Total Produk Keluar"; totalValue = report.summary.totalProductOut; }
  ...
  return <div>
    ...
    <Card><div className="grid gap-4 md:grid-cols-4">
      <div><label className="mb-2 block text-sm font-medium text-gray-700">Jenis Laporan</label><select value={reportType} onChange={(e) => setReportType(e.target.value)} className="w-full rounded-lg border border-gray-300 px-4 py-3 outline-none focus:border-amber-600">{REPORT_TYPES.map((item) => <option key={item.value} value={item.value}>{item.label}</option>)}</select></div>
      <div><label className="mb-2 block text-sm font-medium text-gray-700">Tanggal Mulai</label><input type="date" value={startDate} onChange={(e) => setStartDate(e.target.value)} className="w-full rounded-lg border border-gray-300 px-4 py-3 outline-none focus:border-amber-600" /></div>
      <div><label className="mb-2 block text-sm font-medium text-gray-700">Tanggal Akhir</label><input type="date" value={endDate} onChange={(e) => setEndDate(e.target.value)} className="w-full rounded-lg border border-gray-300 px-4 py-3 outline-none focus:border-amber-600" /></div>
      <div className="flex items-end"><Button onClick={loadReport} disabled={loading} className="w-full">{loading ? "Memuat..." : "Tampilkan Laporan"}</Button></div>
    </div></Card>
    {report && <>

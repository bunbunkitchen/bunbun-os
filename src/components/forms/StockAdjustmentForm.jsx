...
function createOperationKey() {
  ...
  return `stock-adjustment-${Date.now()}-${Math.random()
    .toString(36)
    .slice(2)}`;
}
...
export default function StockAdjustmentForm({
  ...
  function validate() {
    ...
    if (!form.jenis) {
      next.jenis =
        "Jenis adjustment wajib dipilih";
    }
    ...
    if (!form.alasan.trim()) {
      next.alasan =
        "Alasan adjustment wajib dipilih";
    }
  ...
  async function submit(event) {
    ...
    try {
      ...
      setFormError(
        error.message ||
          "Stock adjustment gagal disimpan."
      );
    }
  }
    ...
    >
      ...
      <label className="block space-y-1 text-sm font-medium text-gray-700">
        ...
        <textarea
          ...
          disabled={saving}
          placeholder="Contoh: hasil stock opname tanggal 13 Agustus"
        />
      </label>
      ...
      <div className="rounded-xl bg-amber-50 p-4 text-sm text-amber-900">
        ...
        Adjustment hanya mengubah
        saldo stok. Tidak mencatat
        pengeluaran kas dan tidak
        mengubah HPP resep.
      </div>

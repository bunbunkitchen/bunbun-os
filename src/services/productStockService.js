...
function createUuid() {
  ...
  if (typeof globalThis.crypto?.getRandomValues !== "function") {
    throw new Error(
      "Browser tidak mendukung pembuatan operation key."
    );
  }
...
export function getProductStockErrorMessage(error) {
  ...
  if (!message) {
    return "Proses stok produk gagal. Silakan coba lagi.";
  }

  if (message.includes("Akses operasional")) {
    return "Anda tidak memiliki akses untuk memproses stok produk.";
  }

  if (message.includes("Permintaan ini sudah pernah dicatat")) {
    return "Tindakan ini sudah diproses. Hindari mengirim ulang tindakan yang sama.";
  }

  if (message.includes("Stok frozen lot tidak cukup")) {
    return message;
  }

  if (message.includes("Stok produk jadi tidak cukup")) {
    return message;
  }

  if (message.includes("Produk aktif tidak ditemukan")) {
    return "Produk tidak aktif atau tidak ditemukan.";
  }
...
export async function invokeFrozenFlowRpc(
  functionName,
  params,
  operationKey
) {
...
export async function getFrozenProcessingSplits() {
  const { data, error } = await supabase
    ...
    `)
    .eq("route", "DIRECT")
    .not("source_split_id", "is", null)
    .in("status", ["PROOFING", "BAKING"])
    .eq("is_deleted", false)
    .order("created_at", { ascending: true });
...
export async function getProductStockMovements({
  ...
  if (batchSplitId) {
    query = query.eq("batch_split_id", batchSplitId);
  }
...
export async function getFrozenStockByProductAndLot() {
  const movements = await getProductStockMovements({
    movementTypes: ["FROZEN_IN", "FROZEN_OUT"],
  });

  const lots = new Map();

  movements.forEach((movement) => {
    if (!movement.batchSplitId) {
      return;
    }

    const current = lots.get(movement.batchSplitId) || {
      lotId: movement.batchSplitId,
    ...
    if (movement.tipe === "FROZEN_IN") {
      current.masuk += movement.jumlah;
    }

    if (movement.tipe === "FROZEN_OUT") {
  ...
  return Array.from(lots.values()).sort((first, second) =>
    `${first.productNama}-${first.lotCode}`.localeCompare(
      `${second.productNama}-${second.lotCode}`,
      "id"
    )
  );
}
...
export async function getFinishedProductBalances() {
  ...
  return Array.from(products.values()).sort((first, second) =>
    first.productNama.localeCompare(second.productNama, "id")
  );
}

export async function recordCafeDeposit({
  productId,
  qty,
  movementDate,
  notes,
